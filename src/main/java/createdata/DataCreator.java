...
public class DataCreator {
	 ...
	 * repository này đã tồn tại, làm mới nó rồi mới thêm triple. Duyệt lần lượt
	 * từng phần tử trong entityData, do số phần tử của các thực thể là bằng nhau,
	 * lại được xắp xếp trước. Nên ta có thể tính được thực thể đang xét là dạng nào
	 * (Person, Time ...). Thứ tự tập phần tử là { "Person", "Organization",
	 * "Location", "Event", "Country", "Time" }. Mỗi tập phần tử có
	 * Setting.DEFAULT_INDEX phần tử. Các phép toán như i < 5*Setting.DEFAULT_INDEX
	 * là dùng để xác định xem phần tử đang xét hiện có dạng nào. Từ đó sinh định
	 * danh cho nó. Trong ví dụ trên, phần tử đang xét có thuộc tập phần tử Country
	 * 
...
//		}
		...
		int MaxRel = Setting.REL_INDEX;
		label1: for (int i = 0; i < Max; i++) {
			for (int j = 0; j < Max; j++) {
